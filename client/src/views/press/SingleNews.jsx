...
export default function SingleNews() {
  ...
  useEffect(() => {
    if (slug) {
      fetch(`${SERVER_URL}/api/news/view/${slug}`, {
        credentials: "include",
      })
        .then((response) => {
          if (!response.ok) {
    ...
    fetch(`${SERVER_URL}/api/news/`)
      ...
      .then((data) => {
        ...
        if (Array.isArray(data)) {
          setRecentPosts(data.slice(0, 5)); // Get the 5 most recent posts
        } else {
          ...
          <div className="p-5 basis-3/4 gap-5 flex flex-col">
            {loading ? (
              ...
              <>
                ...
                {post.imageFile && (
                  <img
                    src={`${SERVER_URL}/${post.imageFile}`} // Adjust port if needed
                    alt={post.newsTitle}
                  ...
                  <div key={recentPost._id} className="mb-4">
                    ...
                    >
                      ...
                      <div className="overflow-hidden rounded-xl w-full">
                        <img
                          src={`${SERVER_URL}/${recentPost.imageFile}`} // Ensure the correct path
                          alt={recentPost.newsTitle}
